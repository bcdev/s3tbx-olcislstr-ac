...
import static org.esa.s3tbx.c3solcislstr.mc.lut.ArrayUtilities.degrade;
...
public interface Array {
    ...
    static Array create(Object data) {
        if (data instanceof double[]) {
            return new Array.Double((double[]) data);
        }
        if (data instanceof float[]) {
            return new Array.Float((float[]) data);
        }
    ...
    final class Double implements Array {
        ...
        private Array newArray(int n) {
            return new Array.Double(new double[n]);
        }
    }
    ...
    final class Float implements Array {
        ...
        @NotNull
        private Array newArray(int n) {
            return new Array.Float(new float[n]);
        }
